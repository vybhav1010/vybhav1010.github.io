...
Now, we can zoom into a spot on our conductor. If we pick just a point though, there are no charges to be found! So, we zoom into a little strip $d\bf{l}$. Since the cross-sectional area is $A$, this has volume $A\vert d\bf{l}|$. The number of charges $q$ present is $nA\vert d\bf{l}\vert$, each moving with velocity $\bf{v}$. The force on each is $q\bf{v} × \bf{B}$, so the total force, $d\bf{F}$ is $nqA\vert dl\vert\bf{v} × \bf{B}$.  But, we've defined our $\bf{l}$ vector to be in the same direction as our velocity vector (the direction of the current). So, we may instead take the magnitude of $\bf{v}$ (the drift speed), and replace $\vert \bf{l} \vert$ with $d\bf{l}$ the vector. Our final expression is:
