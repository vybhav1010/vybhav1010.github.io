...
Now, we can zoom into a spot on our conductor. If we pick just a point though, there are no charges to be found! So, we zoom into a little strip $d\bf{l}$. Since the cross-sectional area is $A$, this has volume $A|d\bf{l}|$. The number of charges $q$ present is $nA|d\bf{l}|$, each moving with velocity $\bf{v}$. The force on each is $q\bf{v} × \bf{B}$, so the total force, $d\bf{F}$ is $nqA|dl|\bf{v} × \bf{B}$.  But, we've defined our $\bf{l}$ vector to be in the same direction as our velocity vector (the direction of the current). So, we may instead take the magnitude of $\bf{v}$ (the drift speed), and replace |d$\bf{l}$| with $d\bf{l}$ the vector. Our final expression is:
